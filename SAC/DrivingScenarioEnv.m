classdef DrivingScenarioEnv < rl.env.MATLABEnvironment
    ...
    methods              
        function this = DrivingScenarioEnv()
            ...
            ObservationInfo = rlNumericSpec([13, 1]); % # of state
            ObservationInfo.Name = 'real-time traffic information';
            ObservationInfo.Description = '';

            % Initialize action settings   
            ActionInfo = rlNumericSpec([4, 1],'LowerLimit', 0, 'UpperLimit',1); % three phases
...
%                     notifyEnvUpdated(this);
        ...
        function InitialState = reset(this)
            ...
            % Initialize scenario
            this.scenario = createFourWayJunctionScenario();
            this.scenario.StopTime = 100;
            this.scenario.SampleTime = 0.05;
            this.T = 0;
            % initialize network
            this.network = createFourWayJunctionNetwork(this.scenario);
            this.traffic = trafficControl.TrafficController(this.network(9:20));
            % car parameters
            this.InjectionRate = [250, 250, 250, 250]; % veh/hour
            this.TurnRatio = [50, 50, 50];
            this.cars = createVehiclesForFourWayJunction(this.scenario, this.network, this.InjectionRate, this.TurnRatio);
...
%             notifyEnvUpdated(this);            
    ...
    methods               
        function force = getForce(this,action)
            if ~ismember(action,this.discrete_action)
